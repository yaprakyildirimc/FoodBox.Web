...
$(document).ready(function () {
    ...
    var _transferType = getParameterByName("transferType");
    if (_transferType == "inComing") {
        $("#transferHeader").html("Gelen Transfer");
        $("#btnNewTransfer").hide();
    } else {
        $("#transferHeader").html("Giden Transfer");
        $("#btnNewTransfer").show();
    }
...
function addItem() {
    ...
    var data = $("#transferSaveForm").serializeArray();
...
function editItem(itemId) {
    clearForm("transferSaveForm")

    $("#saveBtn").css("display", "none");
    $("#updateBtn").css("display", "inline-block");
    $("#transferSaveModel").modal("show");

    $.ajax({
        type: "GET",
        url: '/Transfer/GetById',
        data: { Id: itemId },
        success: function (data) {
            var ser = $('#transferSaveForm').serializeArray();
...
function getTransferIncoming() {
    var data = $("").serializeArray();
    data.push({ name: 'id', valueu: userId });

    $.ajax({
        type: "POST",
        url: "/Transfer/getTransferIncoming",
        datatype: "json",
        contentType: "application/x-www-form-urlencoded; charset=UTF-8",
        data: data,
        success: function (data) {
