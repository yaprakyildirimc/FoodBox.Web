...
$(document).ready(function () {
    ...
    itemList = $("#itemList").DataTable({
        "ajax": {
            "url": path + "/GetBasicJsonListOveride",
            "type": "POST",
            "datatype": "json",
            "data": 'data.data'
        },
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, row) {
                    if (row.isCorrect) {
                        return `<span class="label label-lg label-light-success label-inline">Doğru</span>`;
                    } else {
                        return `<span class="label label-lg label-light-danger label-inline">Yanlış</span>`;
                    }
                }
            }
        ]
        ...
        $("#itemList tr td:last-child").each(function (index) {
            if (data.length > 0) {
                if (data[index] != "undefined") {
                    $(this).html(
                        `
                           <a href="#" class="btn btn-light-success font-weight-bolder font-size-sm" onclick="deleteItem(\'` + data[index].id + `\')">
                                Detay
                            </a>
                        `
                    );
